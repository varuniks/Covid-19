...
def get_weekly_covid_data():
    ...
    X.drop(list(X.columns)[1:40], axis=1, inplace=True)
...
#rmse_all = eval_forecasts(Y_t, np.rint(prediction), in_win, out_win)
# rescale
for i in range(len(prediction)):
    prediction[i,:,:] = np.rint(scaler.inverse_transform(prediction[i,:,:]))
    Y_t[i,:,:] = scaler.inverse_transform(Y_t[i,:,:])

e_all = eval_forecasts(Y_t, prediction, in_win, out_win)
print(e_all)
