...
def gen_lstm(in_win, out_win, n_features, dropout, training_t):
    lstm_inputs = Input(shape=(in_win, n_features,))
    l1 = LSTM(50,return_sequences=True)(lstm_inputs)
    l1 = Dropout(0.1)(l1,training=True)
    l1 = LSTM(50,return_sequences=False)(l1)
    l1 = Dropout(0.1)(l1,training=True)
    op = Dense(out_win, activation='linear', name='output')(l1)
...
def gen_EncDec(in_win, out_win, n_features, dropout, training_t):    
    enc_inputs = Input(shape=(in_win, n_features))
    enc_l1 = LSTM(50,return_state=True)
    #enc_l1 = Dropout(0.1)(enc_l1, training=training_t)
    enc_outputs = enc_l1(enc_inputs)
    encoder_states1 = enc_outputs[1:]
    decoder_inputs = RepeatVector(out_win)(enc_outputs[0])
    dec_l1 = LSTM(50, return_sequences=True)(decoder_inputs,initial_state = encoder_states1)
...
model_func = {'lstm':gen_lstm, 'ED':gen_EncDec}
def get_model(model_name, in_win, out_win, n_features, dropout, training_t):
    return model_func[model_name](in_win, out_win, n_features, dropout, training_t)
